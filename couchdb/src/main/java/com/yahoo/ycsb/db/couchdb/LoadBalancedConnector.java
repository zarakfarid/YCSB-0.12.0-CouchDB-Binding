...
public class LoadBalancedConnector implements CouchDbConnector{
	...
	private List<CouchDbConnector> createConnectors(List<URL> urlsForConnectors, String databaseName){
		...
		for(URL url : urlsForConnectors){
			HttpClient httpClient = new StdHttpClient.Builder().url(url).username("admin")
	...
	public CouchDbConnector getConnectorForMutationOperations(){
	...
	public void createBulk(List<Object> o) {
		boolean failed = true;
		for(int i=0; i<this.connectors.size() && failed; i++){
			try{
				this.getConnectorForMutationOperations().executeBulk(o);
				failed = false;
			} catch(UpdateConflictException exc){
				throw exc;
			} catch(Exception exc){}
	...
	public List<DocumentOperationResult> executeBulk(Collection<?> objects) {
		List<DocumentOperationResult> documents = null;
		boolean failed = true;
		for(int i=0; i<this.connectors.size() && failed; i++){
			try{
				documents = this.getConnectorForMutationOperations().executeBulk(objects);
				failed = false;
			} catch(UpdateConflictException exc){
				throw exc;
			} catch(Exception exc){}
